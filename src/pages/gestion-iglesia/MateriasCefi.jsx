...
function MateriasCefi() {
  ...
  async function cargarDatos() {
    ...
    const [
      ...
      supabase
        .from("cefi_materias")
        .select("id, nombre, descripcion, horas_semana")
        .eq("activo", true)
        .order("nombre"),
      ...
      supabase
        .from("cefi_grupos")
        .select(
          "id, nombre, cefi_grados(nombre), cefi_periodos(nombre)"
        )
        .eq("activo", true)
        .order("nombre"),
      ...
      supabase
        .from("cefi_docentes")
        .select("id, nombres, apellidos, miembro_id, perfil_id")
        .eq("estado", "activo")
        .order("nombres"),
      ...
      supabase
        .from("miembros")
        .select(
          "id, nombres, correo_electronico, telefono_movil, activo"
        )
        .eq("activo", true)
        .order("nombres"),
    ]);
    ...
    const error =
      materiasRespuesta.error ||
      gruposRespuesta.error ||
      docentesRespuesta.error ||
      miembrosRespuesta.error;

    if (error) {
      toast.error(`No fue posible cargar configuración: ${error.message}`);
    } else {
      ...
      if (gruposRespuesta.data?.length && !grupoId) {
        setGrupoId(gruposRespuesta.data[0].id);
      }
    }
  ...
  async function cargarAsignaciones() {
    ...
    const { data, error } = await supabase
      .from("cefi_asignaciones")
      .select(
        `
        id,
        materia_id,
        docente_id,
        cefi_materias(nombre),
        cefi_docentes(nombres, apellidos)
        `
      )
      .eq("grupo_id", grupoId)
      .order("created_at");
    ...
    if (error) {
      toast.error(
        `No fue posible cargar materias del grupo: ${error.message}`
      );
    } else {
  ...
  async function crearMateria(evento) {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        descripcion: descripcionMateria.trim() || null,
      });
  ...
  async function convertirMiembroEnProfesor(evento) {
    ...
    try {
      /*
       * 1. Buscamos si ya existe un docente para ese miembro.
       */
      ...
      if (docenteConsultaError) {
        throw docenteConsultaError;
      }

      /*
       * 2. Buscamos el perfil asociado al miembro.
       *
       * No cambiamos el rol general del perfil.
       */
      ...
      if (!perfil) {
        throw new Error(
          "Este miembro no tiene un perfil de acceso asociado. Primero debe tener un perfil en PRO-TEMPLO."
        );
      }

      /*
       * 3. Verificamos si ya tiene un rol CEFI.
       */
      ...
      /*
       * 4. Creamos o activamos el rol Profesor CEFI.
       */
      if (rolCefiExistente) {
        const { error } = await supabase
          .from("cefi_roles_usuario")
      ...
      /*
       * 5. Creamos el docente CEFI si todavía no existe.
       */
      if (!docenteExistente) {
        const { error } = await supabase
          .from("cefi_docentes")
        ...
        /*
         * Si ya existía, lo sincronizamos con el miembro.
         */
        const { error } = await supabase
          .from("cefi_docentes")
          .update({
      ...
      toast.success(
        `${miembro.nombres} ahora es Profesor CEFI.`
      );
  ...
  async function quitarAsignacion(id) {
    ...
    if (error) {
      toast.error(
        `No fue posible quitar la asignación: ${error.message}`
      );
    } else {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mb-6 rounded-3xl bg-slate-950 px-6 py-8 text-white shadow-xl sm:px-10">
          ...
          <p className="mt-2 text-sm text-slate-300">
            La Coordinadora puede seleccionar miembros existentes y
            asignarlos como profesores de CEFI.
          </p>
        </section>
        ...
        <section className="mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Selecciona profesores directamente desde los miembros
                registrados en PRO-TEMPLO.
              </p>
            </div>
          ...
          <form
            onSubmit={convertirMiembroEnProfesor}
          ...
          >
            <label className="text-sm font-semibold text-slate-700">
              ...
              <input
                ...
                onChange={(evento) =>
                  setBusquedaMiembro(evento.target.value)
                }
            ...
            <label className="text-sm font-semibold text-slate-700">
              ...
              <select
                value={miembroId}
                onChange={(evento) =>
                  setMiembroId(evento.target.value)
                }
                required
                disabled={cargando || guardando}
                className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm outline-none focus:border-amber-400"
              >
                <option value="">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-3 text-sm font-bold text-slate-950 disabled:opacity-50"
            >
              <UserPlus size={17} />
              Asignar profesor
            </button>
          ...
          <div className="mt-7 border-t border-slate-100 pt-5">
            <h3 className="font-bold">
              Profesores registrados ({docentes.length})
            </h3>

            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {docentes.length === 0 ? (
                <p className="text-sm text-slate-500">
                  Todavía no hay profesores CEFI.
                </p>
              ) : (
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
            ...
            >
              <input
                value={nombreMateria}
                onChange={(evento) =>
                  setNombreMateria(evento.target.value)
                }
                placeholder="Ej. Fundamentos de la fe"
                required
                className="w-full rounded-xl border border-slate-200 px-3 py-3 text-sm outline-none focus:border-amber-400"
              />

              <textarea
                value={descripcionMateria}
                onChange={(evento) =>
                  setDescripcionMateria(evento.target.value)
                }
              ...
              <button
                type="submit"
                disabled={guardando}
                className="inline-flex items-center gap-2 rounded-xl bg-amber-400 px-4 py-3 text-sm font-bold text-slate-950 disabled:opacity-50"
              >
                <Plus size={17} />
                Guardar materia
              </button>
            ...
            >
              ...
              >
                ...
                {grupos.map((grupo) => (
                  ...
                  >
                    {grupo.nombre} ·{" "}
                    {grupo.cefi_grados?.nombre || "Sin nivel"}
                  </option>
                ))}
              </select>
              ...
              <button
                type="submit"
                disabled={guardando || !grupoId}
                className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-sm font-bold text-white disabled:opacity-50"
              >
                <Plus size={17} />
                Asignar materia
              </button>
              ...
              <div className="mt-3 space-y-2">
                {asignaciones.map((asignacion) => (
                  ...
                  >
                    <div>
                      <div className="font-semibold">
                        {asignacion.cefi_materias?.nombre ||
                          "Materia"}
                      </div>

                      <div className="mt-1 text-xs text-slate-500">
                        {asignacion.cefi_docentes
                          ? `${asignacion.cefi_docentes.nombres} ${
                              asignacion.cefi_docentes.apellidos ||
                              ""
                            }`
                          : "Sin docente"}
                      </div>
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        quitarAsignacion(asignacion.id)
                      }
